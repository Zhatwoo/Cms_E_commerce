...
function MonitoringPageContent() {
  ...
  const submitWebsiteAction = async () => {
    ...
    try {
      ...
      const actionLabel = websiteActionModal.action === 'delete' ? 'Deleted' : 'Taken Down';
      addNotification(`Website ${actionLabel}`, `${actionLabel} ${website.domainName}. Reason: ${reason}`, websiteActionModal.action === 'delete' ? 'error' : 'warning');
  ...
  const submitDeleteProduct = async () => {
    ...
    try {
      ...
      addNotification("Product Deleted", `Deleted product "${product.name}" from ${product.subdomain || 'unknown site'}. Reason: ${reason}`, 'error');
        ...
        <div className="flex min-h-screen flex-1 flex-col">
          ...
          <main className="flex-1 overflow-y-auto">
            <div className="p-6 lg:p-8">
              ...
              {/* ── Dashboard Grid (Shared Sidebar + Tab Content) ─────────────────────────── */}
              <div className="grid grid-cols-1 xl:grid-cols-[480px_minmax(0,1fr)] gap-4 items-start">
                ...
                {/* Right Content Column: Websites or Products */}
                <div className="max-h-[calc(100vh-260px)] overflow-y-auto pr-1">
                  <AnimatePresence mode="wait">
                    {activeTab === 'websites' ? (
                      ...
                      >
                        {loading ? (
                          <p className="text-sm font-medium" style={{ color: '#a090c8' }}>Loading websites…</p>
                        ) : uniqueFilteredWebsites.length === 0 ? (
                          <div className="rounded-3xl p-12 text-center" style={panelStyle}>
                          ...
                          <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
                            {uniqueFilteredWebsites.map((w) => {
                              ...
                              return (
                                <article
                                  key={`${w.userId}::${w.id}`}
                                  className="overflow-hidden rounded-[20px] transition-transform hover:-translate-y-1"
                                  style={{
                                    border: '1.5px solid rgba(166,61,255,0.12)',
                                    boxShadow: '0 4px 20px rgba(103,2,191,0.06)',
                                    background: 'rgba(255,255,255,0.9)',
                                  }}
                                >
                                  {/* Thumbnail */}
                                  <div className="relative h-44 overflow-hidden">
                                    ...
                                    <span className="absolute bottom-3 left-3 rounded-full px-2.5 py-1 text-[11px] font-semibold"
                                      style={{ background: 'rgba(166,61,255,0.1)', color: '#7b1de8', backdropFilter: 'blur(4px)' }}>
                                      {industry}
                                    </span>
                                  </div>
                                  ...
                                  <div className="px-4 py-4">
                                    ...
                                    <div className="flex items-center justify-between gap-3">
                                      <div>
                                        <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: '#a090c8' }}>Publisher</p>
                                      ...
                                      <div className="flex items-center gap-2">
                                        <a href={viewUrl} target="_blank" rel="noopener noreferrer"
                                          ...
                                          style={{ background: 'rgba(166,61,255,0.08)', color: '#7b1de8', border: '1px solid rgba(166,61,255,0.15)' }}>
                                          View
                                        </a>
                                        <button type="button" onClick={() => openWebsiteActionModal(w)}
                                          ...
                                          {workingWebsiteKey === `${w.userId}::${w.id}` ? '…' : 'Dismiss'}
                                        </button>
                                      </div>
                                    </div>
                                  </div>
                      ...
                      >
                        {loading ? (
                          <p className="text-sm font-medium" style={{ color: '#a090c8' }}>Loading products…</p>
                        ) : filteredProducts.length === 0 ? (
                          <div className="w-full rounded-3xl p-12 text-center" style={panelStyle}>
                            <p className="text-sm font-medium" style={{ color: '#7a6aa0' }}>No products found.</p>
                          </div>
                        ) : filteredProducts.map((p) => (
                          <article
                            key={`${p.id}-${p.subdomain || 'site'}`}
                            className="w-[300px] overflow-hidden rounded-[20px] flex flex-col transition-transform hover:-translate-y-1"
                            style={{
                              border: '1.5px solid rgba(166,61,255,0.12)',
                              boxShadow: '0 4px 20px rgba(103,2,191,0.06)',
                              background: 'rgba(255,255,255,0.9)',
                          ...
                          >
                            {/* Product image */}
                            <div className="relative h-52 overflow-hidden flex items-center justify-center" style={{ background: '#f5f3ff' }}>
                              <Image
                                src={(Array.isArray(p.images) && p.images[0]) ? p.images[0] : PRODUCT_CARD_IMAGE}
                                alt={p.name || 'Product'}
                                fill sizes="300px"
                                className="object-contain p-4 scale-105"
                                unoptimized={Array.isArray(p.images) && !!p.images[0]}
                              />
                              <span className="absolute left-2.5 top-2.5 rounded-full px-2.5 py-1 text-[10px] font-bold z-10 shadow-sm"
                                style={{ background: '#FFCC00', color: '#1a1035' }}>
                                {p.subdomain || 'example.com'}
                              </span>
                              <span className="absolute right-2.5 bottom-2.5 rounded-full px-2.5 py-1 text-[10px] font-bold z-10 shadow-sm"
                                style={{ background: 'rgba(255,255,255,0.95)', color: '#7a6aa0', border: '1px solid rgba(166,61,255,0.1)' }}>
                                {productIndustry(p)}
                              </span>
                            </div>
                            ...
                            <div className="px-5 py-4 flex flex-col gap-2 flex-1">
                              <div className="flex items-start gap-2">
                                ...
                                <span className="rounded-full px-2 py-0.5 text-[9px] font-bold shrink-0"
                                  style={{ background: 'rgba(166,61,255,0.08)', color: '#7b1de8', border: '1px solid rgba(166,61,255,0.1)' }}>
                                  {p.subcategory || 'General'}
                                </span>
                              </div>
                              <p className="text-[10px] uppercase font-bold tracking-widest" style={{ color: '#a090c8' }}>SKU: {p.sku || 'N/A'}</p>
                              <div className="flex items-center gap-3 mt-auto pt-2">
                                <button type="button" onClick={() => setSelectedProduct(p)}
                                  className="rounded-xl px-4 py-2 text-xs font-bold transition hover:brightness-95"
                                  style={{ background: 'rgba(166,61,255,0.08)', color: '#7b1de8', border: '1px solid rgba(166,61,255,0.15)' }}>
                                  View
                                </button>
                                <button type="button" onClick={() => openDeleteProductModal(p)}
                                  disabled={workingProductId === p.id}
                                  className="rounded-xl px-4 py-2 text-xs font-bold text-white transition hover:brightness-95 disabled:opacity-60"
                                  style={{ background: '#ef4444' }}>
                                  Delete
                                </button>
                                <span className="ml-auto text-sm font-bold" style={{ color: '#c89000' }}>
