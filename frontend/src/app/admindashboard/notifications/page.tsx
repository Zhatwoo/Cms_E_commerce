...
function NotificationsPageContent() {
  ...
  const handleDelete = () => {
    ...
    setTrash((current) => [
      ...toTrash.filter((item) => !current.some((entry) => entry.id === item.id)),
      ...current,
    ]);
                  ...
                  {([
                    ...
                    <button
                      ...
                      className={`min-w-[130px] rounded-[10px] px-6 py-3 text-[1rem] font-semibold transition ${
                        activeTab === tab.key ? "bg-[#FFCC00] text-[#2F1859]" : "text-[#787593] hover:bg-white/60"
                      }`}
                    >
                    ...
                    {activeTab === "list" && (
                      ...
                      >
                        ...
                        <div className="mb-7 flex flex-wrap items-center gap-5">
                          ...
                          <ActionButton onClick={() => void handleMarkAsRead()} disabled={selectedIds.length === 0} icon={<CheckIcon />}>
                            Mark as Read
                          </ActionButton>
                          <ActionButton onClick={() => void handleMarkAllAsRead()} disabled={notifications.every((n) => n.read)} icon={<CheckIcon />}>
                            Mark all as Read
                          </ActionButton>
                            ...
                            {notifications.map((item) => (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-5 pl-4 text-sm text-[#8B85A5]">
                                  ...
                                  <span className={item.read ? "text-[#1AA54B]" : "text-[#FF5252]"}>{item.read ? <CheckIcon /> : "x"}</span>
                                </div>
                              </motion.div>
                            ))}
                          </div>
