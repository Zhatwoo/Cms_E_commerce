    ...
    <div className={asPage ? 'w-full py-1' : 'fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4'}>
      <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.22, ease: [0.25, 0.46, 0.45, 0.94] }} className={asPage ? 'relative w-full flex flex-col' : 'relative w-full max-w-2xl rounded-2xl border border-[#1F1F51] bg-[#0E0C30] shadow-2xl overflow-hidden flex flex-col'} style={asPage ? undefined : { maxHeight: '85vh' }}>
        <div className={`px-7 pt-7 pb-5 flex items-start justify-between shrink-0 ${asPage ? '' : 'border-b border-[#1F1F51]'}`}>
          ...
          {view === 'create' ? (
            <button onClick={() => { setView('select'); setError(''); }} className="flex items-center gap-1.5 text-sm font-medium text-[#8A8FC4] hover:text-white transition-colors mt-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg> Back</button>
          ) : (
            ...
            <AnimatePresence mode="wait">
              {view === 'select' && (
                <motion.div key="select" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.18 }}>
                  ...
                  {loading && projectTab === 'active' ? (
                    ...
                    <div className={`grid gap-4 ${viewMode === 'grid' ? (asPage ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3') : 'grid-cols-1'}`}>
                        ...
                        return (
                          <div key={project.id} className={`relative border border-[#2A2A60] bg-[#141140] overflow-hidden ${asPage ? 'rounded-2xl' : 'rounded-2xl'} hover:bg-[#1A1750] hover:border-[#6B72D8] transition-all ${activeMenuProjectId === project.id ? 'z-40' : 'z-0'} 
                            ...
                            {projectTab === 'active' && editingProjectId !== project.id && !project.isShared && (
                              <div className="absolute top-3 right-3 z-50" onClick={(e) => e.stopPropagation()}>
                                ...
                                {activeMenuProjectId === project.id && (
                                  <div className="absolute right-0 mt-2 w-40 rounded-xl border border-[#2A2A60] bg-[#0F0D38] py-1 shadow-2xl z-50">
                                    <button type="button" onClick={(e) => { e.stopPropagation(); startEditProject(project); }} className="w-full px-4 py-2 text-left text-sm text-white flex items-center gap-2 hover:bg-white/5"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 3.487a1.875 1.875 0 1 1 2.652 2.652L8.25 17.403 4.5 18.75l1.347-3.75L16.862 3.487Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 4.5l3.75 3.75" /></svg> Edit Details</button>
                                    <button type="button" onClick={(e) => { e.stopPropagation(); handleMoveToTrash(project); }} disabled={actioningProjectId === project.id} className="w-full px-4 py-2 text-left text-sm text-red-400 flex items-center gap-2 hover:bg-red-500/10 disabled:opacity-50">
                            ...
                            {editingProjectId === project.id ? (
                              ...
                              <div role="button" tabIndex={0} onClick={() => projectTab === 'active' && openProject(project.id)} className={`${projectTab === 'trash' ? 'cursor-default' : 'cursor-pointer'} ${viewMode === 'list' ? 'flex flex-row items-center w-full h-full' : 'w-full text-left group flex flex-col h-full'}`}>
                                <div className={`${viewMode === 'list' ? 'w-40 h-full border-r border-[#2A2A60] shrink-0' : 'w-full aspect-[16/10] border-b border-[#2A2A60]'} overflow-hidden bg-[#0A0826] ${projectTab === 'trash' ? 'grayscale opacity-75' : ''}`}>
                                  {project.thumbnail && projectTab !== 'trash' ? (
                                    ...
                                    <DraftPreviewThumbnail projectId={project.id} borderColor="transparent" bgColor="#0A0826" className={`w-full h-full !aspect-auto !rounded-none object-cover ${projectTab !== 'trash' && 'transition-transform duration-500 group-hover:scale-105'}`} />
                                  )}
                                </div>
              ...
              {view === 'create' && (
                <motion.div key="create" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 8 }} transition={{ duration: 0.18 }}>
                  <form onSubmit={handleCreate} className="space-y-5 max-w-xl mx-auto">
                    <div><label className="block text-sm font-medium text-[#C4C6E8] mb-1.5">Project title</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="My Awesome Store" autoFocus className="w-full px-4 py-3 rounded-xl border border-[#2A2A60] bg-[#0A0826] text-white text-sm placeholder:text-[#4A4A7E] focus:outline-none focus:border-[#6B72D8] transition-colors shadow-inner" /></div>
                    <div>
                      <label htmlFor="project-industry" className="block text-sm font-medium text-[#C4C6E8] mb-1.5">Industry / Store type</label>
                      <select id="project-industry" data-industry-select="true" value={industry} onChange={(e) => setIndustry(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#2A2A60] bg-[#0A0826] text-white text-sm focus:outline-none focus:border-[#6B72D8] transition-colors shadow-inner" required>
                        <option value="" disabled>Select industry</option>
                        {INDUSTRY_OPTIONS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#C4C6E8] mb-1.5">Preferred subdomain <span className="text-[#6B6FA0] font-normal">(optional)</span></label>
                      <input type="text" value={subdomain} onChange={(e) => setSubdomain(e.target.value)} placeholder="mystore" className="w-full px-4 py-3 rounded-xl border border-[#2A2A60] bg-[#0A0826] text-white text-sm placeholder:text-[#4A4A7E] focus:outline-none focus:border-[#6B72D8] transition-colors shadow-inner" />
                      <p className="text-xs text-[#6B6FA0] mt-2">Only letters, numbers, and hyphens. e.g. <span className="text-[#8A8FC4]">mystore → mystore.yourdomain.com</span></p>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-[#1F1F51] mt-6">
                      <button type="button" onClick={() => { setView('select'); setError(''); }} className="px-5 py-2.5 rounded-xl text-sm font-medium text-[#8A8FC4] hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
                      <button type="submit" disabled={creating} className="px-6 py-2.5 rounded-xl text-sm font-semibold bg-[#FFCE00] text-[#121241] hover:bg-[#FFD740] disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg shadow-[#FFCE00]/20 flex items-center justify-center min-w-[140px]">{creating ? <span className="flex items-center gap-2"><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Creating...</span> : 'Create & open'}</button>
                    </div>
                  </form>
                </motion.div>
              )}
      ...
      <AnimatePresence>
        {confirmModal.isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 15 }} transition={{ duration: 0.2, type: 'spring', bounce: 0.25 }} className={`w-full max-w-[400px] rounded-3xl border ${confirmModal.variant === 'danger' ? 'border-red-500/30' : confirmModal.variant === 'primary' ? 'border-[#2D3A90]/50' : 'border-yellow-500/30'} bg-[#0A0826] shadow-2xl overflow-hidden flex flex-col`}>
