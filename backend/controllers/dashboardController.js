    ...
    const responseData = {
      ...
      analytics: {
        ...
        trends: {
          ...
          pending: new Array(period === '7days' ? 7 : (period === '30days' ? 4 : 3)).fill(pendingWebsites || 0)
        },
        ...
        signupsOverTime: {
          ...
          signups: signupsOverTime?.signups && signupsOverTime.signups.some(v => v > 0) ? signupsOverTime.signups : [0, 0, 0, 0, 0, 0, activeUsers || 0]
        },
        revenueOverTime: {
          labels: revenueOverTime?.labels && revenueOverTime.labels.length > 0 ? revenueOverTime.labels : ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
          data: revenueOverTime?.data && revenueOverTime.data.some(v => v > 0) ? revenueOverTime.data : [0, 0, 0, 0, 0, 0, totalRevenue || 0]
        },
        workspace: {
